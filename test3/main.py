...
# gumbel
parser.add_argument('--gb-hidden', type=int, default=128)
parser.add_argument('--gb-layer', type=int, default=3)
# g theta
...
config_list = [args.name, args.dataset, args.epochs, args.batch_size,
				args.lr, args.lr_term, args.lr_inc, args.device, 'inp', args.channel_size] + data_config + \
				...
				'gb', args.gb_hidden, args.gb_layer,
...
gb_layout = [((args.cv_filter + 2) + args.te_embedding)] + [args.gb_hidden for i in range(args.gb_layer - 1)] + [1]
...
gumbel = model.MLP(gb_layout).to(device)
...
if args.load_model != '000000000000':
	...
	gumbel.load_state_dict(torch.load(args.log_directory + args.name + '/' + args.load_model + '/gumbel.pt'))
...
optimizer = optim.Adam(list(conv.parameters()) + list(text_encoder.parameters()) + list(gumbel.parameters()) + list(g_theta.parameters()) + list(f_phi.parameters()), lr=args.lr)
...
def encode(objects, code):
	...
	return color_encoded, question_encoded


def pair(color_encoded, question_encoded, logits):
	selection = F.gumbel_softmax(logits.squeeze(2), hard=True)
	selected = torch.bmm(selection.unsqueeze(1), color_encoded).expand_as(color_encoded)
	...
	for batch_idx, (image, question, answer) in enumerate(test_loader):
		...
		if batch_idx == 0:
			...
			if args.dataset == 'clevr':
				...
				image = F.pad(image[:n], (0, 0, 0, args.input_h//2), mode='constant', value=1).transpose(1,2).transpose(2,3)
				...
				for i in range(n):
					...
					cv2.putText(image[i], '{} {} {} {}'.format(
						...
						(2, args.input_h + args.input_h//2), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 0))
