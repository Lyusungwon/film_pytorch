...
def test(epoch):
	...
	writer.add_scalar('Test non-rel accuracy', sum(list(q_correct.values())[:train_loader.dataset.q_size//2]) / sum(list(q_num.values())[:train_loader.dataset.q_size//2]), epoch)
	writer.add_scalar('Test rel accuracy', sum(list(q_correct.values())[train_loader.dataset.q_size//2:]) / sum(list(q_num.values())[train_loader.dataset.q_size//2:]), epoch)
